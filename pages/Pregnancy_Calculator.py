...
def get_summary_details_dataframe(due_date, pregnancy_duration, last_menstrual_period_date):
    ...
    months_pregnant = str(round(float(weeks_pregnant) / 4.345, 2))  # Approximate conversion from weeks to months
    weeks_left = calculate_weeks_left(due_date)
    months_left = str(round(float(weeks_left) / 4.345, 2))
    data = {
        "Description": [
...
def app():
    ...
    st.markdown(f"*Feel free to call this page with some prefilled data to get results easier eg: https:// ...... /Pregnancy_Calculator{quick_url}*")
    ...
    quick_url = f"?calculate_by_option={radiobutton_calculate_by}"
    if radiobutton_calculate_by == "Last Menstrual Period (Start Date)":    
        ...
        quick_url += f"&date={last_menstral_date}"
        ...
        quick_url += f"&date={date_of_conception}"
        ...
        quick_url += f"&date={date_of_ivf_transfer}"
        ...
        quick_url += f"&embryo={radiobutton_embryo}"
